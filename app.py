...
def handle_requests():
    ...
    if not SECRET_KEY or access_key != SECRET_KEY:
        return jsonify({"error": "Access denied. Invalid or missing key uiuiiuy8i9."}), 403

    if not uid or not server_name:
        return jsonify({"error": "UID and server_name are required 5h5h5"}), 400

    try:
        tokens = load_tokens(server_name)
        if not tokens: return jsonify({"error": "Could not load tokens for the server 6tn66j."}), 500
